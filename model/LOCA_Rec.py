...
class LOCA_Rec(BaseRecommender):
    def __init__(self, dataset, model_conf, device):
        super(LOCA_Rec, self).__init__(dataset, model_conf)
    ...
    def train_single_model(self, local_num):
        ...
        local_model = LocalRec(local_num, train_weight, test_weight, self.candidate_users[local_num],
    ...
    def predict(self, user_ids, eval_pos_matrix, eval_items=None):
        ...
        for local_num in range(self.num_local):
            ...
            train_weight = torch.tensor(self.kernel_matrix[local_num][0])  # vector
            test_weight = torch.tensor(self.kernel_matrix[local_num][2])  # vector
            local_model = LocalRec(local_num, train_weight, test_weight, self.candidate_users[local_num], self.dataset, self.model_type, self.model_conf, self.device)
