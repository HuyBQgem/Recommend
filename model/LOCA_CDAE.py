...
class LOCA_CDAE(BaseRecommender):
    def __init__(self, dataset, model_conf, device):
        super(LOCA_CDAE, self).__init__(dataset, model_conf)
    ...
    def train_single_model(self, local_num):
        ...
        local_model = LocalCDAE(local_num, train_weight, test_weight, self.candidate_users[local_num],
    ...
    def predict(self, user_ids, eval_pos_matrix, eval_items=None):
        ...
        for local_num in range(self.num_local):
            ...
            train_weight = torch.tensor(self.kernel_matrix[local_num][0])  # vector
            test_weight = torch.tensor(self.kernel_matrix[local_num][2])  # vector
            local_model = LocalCDAE(local_num, train_weight, test_weight, self.candidate_users[local_num], self.dataset, self.model_type, self.model_conf, self.device)
    ...
    def build_kernel_matrix(self):
        # for each local modeleval_output
...
class LocalCDAE(BaseRecommender):
    def __init__(self, local_num, train_weight, test_weight, candidate_users, dataset, model_type, model_conf, device):
        ...
        self.user_embedding_CDAE = nn.Embedding(self.num_users, self.hidden_dim)
    ...
    def forward(self, user_id, rating_matrix):
        ...
        enc = self.encoder(normalized_rating_matrix) + self.user_embedding_CDAE(user_id)
    ...
    def train_model_per_batch(self, batch_idx, batch_matrix, batch_weight=None):
        ...
        if batch_weight is None:
            ...
            batch_loss = (F.binary_cross_entropy(pred_matrix, batch_matrix, reduction='sum') * batch_weight.view(pred_matrix.shape[0], -1)).mean()
    ...
    def train_model(self, dataset, evaluator, early_stop, logger, config):
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            for b, batch_idx in enumerate(batch_loader):
                ...
                batch_train = torch.tensor(batch_train).to(self.device)

                batch_loss = self.train_model_per_batch(batch_train, batch_input, batch_weight)
    ...
    # def predict(self, user_ids, eval_pos_matrix, eval_items=None):
    #     self.eval()
    #     eval_output = self.predict(user_ids, eval_pos_matrix, eval_items)
    #     self.train()
    ...
    def predict(self, user_ids, eval_pos_matrix, eval_items=None):
        ...
        with torch.no_grad():
            ...
            # user_ids = torch.as_tensor(np.array(user_ids).astype('float')).to(self.device)
